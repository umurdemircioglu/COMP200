...
;_BANNER_
;;;
;;; umurdemircioglu17@ku.edu.tr     11/11/2019
;;;
;;; Before you start:
;;;
...
(define (segment-length seg)
  (define x1 (x-of (start-seg seg)))
  (define x2 (x-of (end-seg seg)))
  (define y1 (y-of (start-seg seg)))
  (define y2 (y-of (end-seg seg)))
...
(define (create-polygon ptlist)
  (define p (append ptlist (list (car ptlist))))
  (define end (list))
  (polygon-helper p end)
)
  (define (polygon-helper  ptlist end )
    (if (null? (cdr ptlist)) end
      (append end (list (make-seg (first ptlist) (second ptlist))) (polygon-helper(cdr ptlist)end))))
...
(define x (+ (x-of p1) (x-of p2)))
(define y (+ (y-of p1) (y-of p2)))
(make-point x y)
)
...
(define (add-all-points lst)
 (add-all-points-helper lst (make-point 0 0))                
  )

(define (add-all-points-helper lst sum)
(if (null? lst) sum
   (add-all-points-helper (cdr lst) (add-two-points sum (car lst)))))
; test
...
(define (centroid lst)
  (centroid-helper lst (add-all-points lst) 0)
)
(define (centroid-helper lst sum n)
  (if (null? lst) (make-point (/ (x-of sum) n) (/ (y-of sum) n))
  (centroid-helper (cdr lst) sum (+ n 1))))
...
(define (centroid-alt lst)
 (make-point (/ (sum(listofx lst)) (length lst)) (/ (sum (listofy lst)) (length lst))))


(define (listofx lst)
(listofx-helper lst (list)))

(define (listofx-helper lst lstfinal)
    (if (null? lst) lstfinal
    (listofx-helper (cdr lst) (append lstfinal (list (x-of (car lst)) )))))
  

(define (listofy  lst)
  (listofy-helper lst (list)))


 (define (listofy-helper lst lstfinal)
    (if (null? lst) lstfinal
    (listofy-helper (cdr lst) (append lstfinal (list (y-of (car lst)) )))))
...
;If we evaluate this it gives a contract violation because our function square expects a number inside.
...
; into operations on proc edures. The method for making such operations is
; captured by
...
;Dr Racket gives a contract violations because it expects a number but we gave a procedure.
...
(lambda (t) (define scale1 (* t len)) (make-point (* scale1 (sin (/ 3.1415929 4))) (* scale1 (cos (/ 3.1415929 4))))))
...
((diagonal-line p1 (* 3 (sqrt 3))) 0)
;(0 0)
((diagonal-line p1 (* 3 (sqrt 3))) 1)
;(3.674234840516988 3.674234387832533)
...
(lambda (t) (make-point (x-of (curve t)) (* -1 (y-of (curve t))))))
...
(define (connect-ends curve1 curve2)
  (lambda (t)(make-point (+ (x-of (curve1 t)) (x-of (curve2 t)))(+ (y-of (curve1 t)) (y-of (curve2 t))))))
...
;draw-connected connects the dots to make it look like a straight line, draw-points draws the points but does not connect them. 
...
;I cooperated with Altay Atalay 64568.
